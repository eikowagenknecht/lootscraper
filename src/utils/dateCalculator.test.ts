...
describe.concurrent("Date Calculator", () => {
  ...
  test("should use valid_to if still current", ({ expect }) => {
    const seenLast = DateTime.fromISO("2020-06-01T00:00:00Z");
    const validTo = DateTime.fromISO("2020-06-15T00:00:00Z");
    const now = DateTime.fromISO("2020-06-01T00:00:01Z");
  ...
  test("should use valid_to if still within timeframe", ({ expect }) => {
    const seenLast = DateTime.fromISO("2020-06-01T00:00:00Z");
    const validTo = DateTime.fromISO("2020-06-01T06:00:00Z");
    const now = DateTime.fromISO("2020-06-01T02:00:00Z");
  ...
  test("should use seen_last if past timeframe", ({ expect }) => {
    const seenLast = DateTime.fromISO("2020-06-01T00:00:00Z");
    const validTo = DateTime.fromISO("2020-06-01T06:00:00Z");
    const now = DateTime.fromISO("2020-06-02T01:00:00Z");
