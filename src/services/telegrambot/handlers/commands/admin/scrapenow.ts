...
export async function handleScrapeNowCommand(ctx: CommandContext<BotContext>): Promise<void> {
  logCall(ctx);
  ...
  if (args) {
    ...
    if (queuedName) {
      ...
      await ctx.reply(`Scraper "${queuedName}" has been queued for immediate scraping.`);
    } else {
      const enabledNames = getEnabledScraperNames().join(", ");
      await ctx.reply(`Unknown scraper "${args}".\n\nAvailable scrapers: ${enabledNames}`);
    }
    return;
  }
  ...
  await ctx.reply("All enabled scrapers have been queued for immediate scraping.");
}
