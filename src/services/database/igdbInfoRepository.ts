...
export async function getIgdbInfoById(id: number): Promise<IgdbInfo | null> {
  try {
    return (
      (await getDb().selectFrom("igdb_info").where("id", "=", id).selectAll().executeTakeFirst()) ??
      null
    );
  } catch (error) {
...
async function updateIgdbInfo(id: number, info: IgdbInfoUpdate): Promise<void> {
  try {
    await getDb().updateTable("igdb_info").set(info).where("id", "=", id).executeTakeFirst();
  } catch (error) {
...
export async function createIgdbInfo(info: NewIgdbInfo): Promise<number> {
  ...
  try {
    const result = await getDb().insertInto("igdb_info").values(info).executeTakeFirstOrThrow();
