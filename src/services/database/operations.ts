...
export class DatabaseOperations {
  ...
  // Game operations
  async findOrCreateGame(steamId?: number | null, igdbId?: number | null) {
    try {
      ...
      const existingGame = await this.db
        ...
        .where((eb) =>
          eb.or([
            steamId ? eb("steam_id", "=", steamId) : eb("steam_id", "is", null),
            igdbId ? eb("igdb_id", "=", igdbId) : eb("igdb_id", "is", null),
          ]),
        )
        .executeTakeFirst();
  ...
  async createAnnouncement(
    announcement: Omit<Database["announcements"], "id">,
  ) {
    try {
      return await this.db
