...
SCRAPER_NAME = "Steam"
ROOT_URL = "https://steamdb.info/upcoming/free/"
MAX_WAIT_SECONDS = 60  # Needs to be quite high in Docker for first run

XPATH_OFFERS = (
    """//tbody//tr[not(@hidden) and @data-appid]"""  # /@data-appid is the steam id
)

SUBPATH_TITLE = """.//td[3]//b"""  # /text()
SUBPATH_TIME_FROM = """.//td[6]"""  # /@data-time  # format 2022-02-24T16:00:00+00:00
SUBPATH_TIME_TO = """.//td[7]"""  # /@data-time  # format 2022-02-24T16:00:00+00:00
SUBPATH_URL = """.//td[4]//a"""  # /@href
SUBPATH_IMG = """.//td[@class="applogo"]//img"""  # /@src
...
class SteamScraper(Scraper):
    ...
    def scrape(options: dict[str, bool] = None) -> dict[str, list[LootOffer]]:
        ...
        try:
            ...
            with get_pagedriver() as driver:
                ...
                offers[OfferType.GAME.name] = SteamScraper.read_offers_from_page(
                    driver
                )
    ...
    def read_offers_from_page(driver: WebDriver) -> list[LootOffer]:
        try:
            ...
            WebDriverWait(driver, MAX_WAIT_SECONDS).until(
                EC.presence_of_element_located((By.XPATH, XPATH_OFFERS))
            )
        ...
        try:
            elements = driver.find_elements(By.XPATH, XPATH_OFFERS)
        except WebDriverException:  # type: ignore
    ...
    def read_raw_offer(element: WebElement) -> RawOffer:
        ...
        try:
            steam_id_int = int(element.get_attribute("data-appid"))  # type: ignore
        except WebDriverException:  # type: ignore
            # Nothing to do here, string stays empty
            pass
    ...
    def normalize_offers(raw_offers: list[RawOffer]) -> list[LootOffer]:
        ...
        for offer in raw_offers:
            ...
            if offer.steam_id:
                rawtext += f"<steamid>{offer.steam_id}</steamid>"
